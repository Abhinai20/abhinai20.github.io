...
// ---------- Multi-Doc YAML Splitter ----------
document.getElementById('yamlsplit-split-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('yamlsplit-result');
  try {
    const input = document.getElementById('yamlsplit-input').value;
    if (!input.trim()) throw new Error('Paste some multi-document YAML first.');
    const docs = jsyaml.loadAll(input);
    if (!docs.length) throw new Error('No documents found.');
    const rendered = docs.map((doc, i) => {
      const kind = doc && doc.kind ? doc.kind : '(unknown kind)';
      const name = doc && doc.metadata && doc.metadata.name ? doc.metadata.name : '(unnamed)';
      return `# --- Document ${i + 1}: ${kind}/${name} ---\n` + jsyaml.dump(doc);
    });
    resultEl.className = 'result-box result-success tf-output';
    resultEl.textContent = `Found ${docs.length} document(s).\n\n` + rendered.join('\n');
  } catch (e) {
    resultEl.className = 'result-box result-error tf-output';
    resultEl.textContent = e.message;
  }
...
// ---------- Toleration / Taint Matcher ----------
function tolerationMatchesTaint(toleration, taint) {
...
document.getElementById('tolerationmatch-test-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('tolerationmatch-result');
  try {
    const tolerationRaw = document.getElementById('tolerationmatch-toleration').value;
    const taintRaw = document.getElementById('tolerationmatch-taint').value;
    if (!tolerationRaw.trim() || !taintRaw.trim()) throw new Error('Fill in both the toleration and taint JSON.');
    const toleration = JSON.parse(tolerationRaw);
    const taint = JSON.parse(taintRaw);
    if (!taint.effect) throw new Error('Taint JSON must include an "effect" field.');
    const matches = tolerationMatchesTaint(toleration, taint);
    resultEl.className = matches ? 'result-box result-success' : 'result-box result-error';
    resultEl.textContent = matches
      ? 'MATCH — this toleration lets the pod be scheduled onto a node with this taint.'
      : 'NO MATCH — this toleration does NOT cover this taint. The pod would be repelled/evicted depending on the taint\'s effect.';
  } catch (e) {
    resultEl.className = 'result-box result-error';
    resultEl.textContent = 'Invalid input: ' + e.message;
  }
});

// ---------- .env Diff Tool ----------
function parseEnvSimple(text) {
  const map = {};
  for (const line of text.split('\n')) {
    const trimmed = line.trim();
...
function diffEnvFiles(a, b) {
  const mapA = parseEnvSimple(a);
  const mapB = parseEnvSimple(b);
  const allKeys = new Set([...Object.keys(mapA), ...Object.keys(mapB)]);
  const added = [], removed = [], changed = [];
  for (const key of allKeys) {
    const inA = Object.prototype.hasOwnProperty.call(mapA, key);
    const inB = Object.prototype.hasOwnProperty.call(mapB, key);
    if (inA && !inB) removed.push(key);
    else if (!inA && inB) added.push(key);
    else if (mapA[key] !== mapB[key]) changed.push({ key, a: mapA[key], b: mapB[key] });
  }
  return { added, removed, changed };
}
document.getElementById('envdiff-compare-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('envdiff-result');
  try {
    const a = document.getElementById('envdiff-a').value;
    const b = document.getElementById('envdiff-b').value;
    if (!a.trim() || !b.trim()) throw new Error('Fill in both .env files.');
    const { added, removed, changed } = diffEnvFiles(a, b);
    const lines = [];
    if (added.length) lines.push('Only in B:\n' + added.map((k) => `  + ${k}`).join('\n'));
    if (removed.length) lines.push('Only in A:\n' + removed.map((k) => `  - ${k}`).join('\n'));
    if (changed.length) lines.push('Changed:\n' + changed.map((c) => `  ~ ${c.key}: "${c.a}" -> "${c.b}"`).join('\n'));
    resultEl.className = (added.length || removed.length || changed.length) ? 'result-box result-error' : 'result-box result-success';
    resultEl.textContent = lines.length ? lines.join('\n\n') : 'No differences — both files define the same keys with the same values.';
  } catch (e) {
    resultEl.className = 'result-box result-error';
    resultEl.textContent = e.message;
  }
...
document.getElementById('storageunit-convert-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('storageunit-result');
  try {
    const value = parseFloat(document.getElementById('storageunit-value').value);
    if (Number.isNaN(value)) throw new Error('Enter a numeric value.');
    const fromUnit = document.getElementById('storageunit-from').value;
    const bytes = value * STORAGE_UNITS[fromUnit];
    const lines = Object.keys(STORAGE_UNITS).map((unit) => {
      const converted = bytes / STORAGE_UNITS[unit];
      return `${unit.padEnd(4)} = ${converted.toLocaleString('en-US', { maximumFractionDigits: 6 })}`;
    });
    resultEl.className = 'result-box result-success';
    resultEl.textContent = `${value} ${fromUnit} = ${bytes.toLocaleString('en-US')} bytes\n\n` + lines.join('\n');
  } catch (e) {
    resultEl.className = 'result-box result-error';
    resultEl.textContent = e.message;
  }
...
document.getElementById('durationdiff-calc-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('durationdiff-result');
  try {
    const aRaw = document.getElementById('durationdiff-a').value.trim();
    const bRaw = document.getElementById('durationdiff-b').value.trim();
    if (!aRaw || !bRaw) throw new Error('Enter both timestamps.');
    const a = new Date(aRaw);
    const b = new Date(bRaw);
    if (isNaN(a.getTime()) || isNaN(b.getTime())) throw new Error('Could not parse one or both timestamps.');
    ...
    resultEl.textContent = [
      ...
      `End:   ${b.toISOString()}`,
      ``,
      `Duration: ${humanDuration(diffMs)}`,
      `Total seconds: ${(diffMs / 1000).toLocaleString('en-US')}`,
      `Total minutes: ${(diffMs / 60000).toLocaleString('en-US', { maximumFractionDigits: 2 })}`,
    ].join('\n');
  } catch (e) {
    resultEl.className = 'result-box result-error';
    resultEl.textContent = e.message;
...
const PORT_REFERENCE = {
  ...
  25: 'SMTP (mail transfer)', 53: 'DNS', 67: 'DHCP (server)', 68: 'DHCP (client)',
  80: 'HTTP', 110: 'POP3', 123: 'NTP (time sync)', 143: 'IMAP',
  161: 'SNMP', 194: 'IRC', 389: 'LDAP', 443: 'HTTPS',
  445: 'SMB (Windows file sharing)', 465: 'SMTPS', 500: 'IKE (IPsec VPN)',
  514: 'Syslog', 587: 'SMTP (submission)', 631: 'IPP (printing)',
  636: 'LDAPS', 993: 'IMAPS', 995: 'POP3S',
  1433: 'Microsoft SQL Server', 1521: 'Oracle DB', 1883: 'MQTT',
  2049: 'NFS', 2181: 'ZooKeeper', 2375: 'Docker daemon (unencrypted)',
  2376: 'Docker daemon (TLS)', 2379: 'etcd (client)', 2380: 'etcd (peer)',
  3000: 'Common dev server (Node/Grafana default)', 3306: 'MySQL/MariaDB',
  3389: 'RDP (Windows Remote Desktop)', 5000: 'Common dev server (Flask/Docker registry)',
  5432: 'PostgreSQL', 5601: 'Kibana', 5672: 'RabbitMQ (AMQP)',
  5900: 'VNC', 6379: 'Redis', 6443: 'Kubernetes API server',
  8080: 'Common HTTP alt / proxy port', 8443: 'Common HTTPS alt port',
  8500: 'Consul', 8888: 'Common Jupyter/dev port', 9000: 'Common app/MinIO port',
  9092: 'Kafka', 9093: 'Alertmanager', 9200: 'Elasticsearch (HTTP)',
  9300: 'Elasticsearch (transport)', 9090: 'Prometheus', 10250: 'Kubelet API',
  27017: 'MongoDB',
};
document.getElementById('portref-lookup-btn').addEventListener('click', () => {
  const resultEl = document.getElementById('portref-result');
  try {
    const raw = document.getElementById('portref-input').value.trim();
    const port = Number(raw);
    if (!raw || Number.isNaN(port) || port < 0 || port > 65535) throw new Error('Enter a valid port number (0-65535).');
    const known = PORT_REFERENCE[port];
    const range = port < 1024 ? 'Well-known port (0-1023)' : port < 49152 ? 'Registered port (1024-49151)' : 'Dynamic/private port (49152-65535)';
    resultEl.className = 'result-box result-success';
    resultEl.textContent = known
      ? `${port}\nCommonly used for: ${known}\n${range}`
      : `${port}\nNo common service on file for this exact port.\n${range}`;
  } catch (e) {
    resultEl.className = 'result-box result-error';
    resultEl.textContent = e.message;
  }
